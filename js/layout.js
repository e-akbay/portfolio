...
$(document).ready(function() {                
    ...
    InitActiveMenuBorder($('#menu-portfolio'));   
...
function OnHashChange(hash)
{
    if(!GlobalHashCustomSet)
    {
        ...
        var menuItem = $('#menu-' + hash);
        if (menuItem.length === 0) {
            hash = 'portfolio';
            menuItem = $('#menu-portfolio');
        }
        $('#active-border').css('left', menuItem.offset().left);
        $('#active-border').css('width', menuItem.width());
...
function LoadContent(content, animate, toRight, hash)
{
    ...
    if(hash)
    {
        ...
        document.title = 'Emir Serhan Akbay - ' + content.charAt(0).toUpperCase() + content.slice(1);
    }
